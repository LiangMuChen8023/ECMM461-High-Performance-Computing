...
/* Buffer space for sending and receiving data in task1 */
int task1_buffer[N_IM+3];
...
int main(int argc, char *argv[]){
  ...
  if ( myRank == 0 ){
    ...
    for (i=0; i<N_RE+1; i++){
      ...
      MPI_Recv(task1_buffer, N_IM+3, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
      nextProc = task1_buffer[0];
      if(task1_buffer[1] != -1){
        for (j=0; j<N_IM+1; j++){
            nIter[task1_buffer[1]][j]=task1_buffer[2+j];
            j++;
        }
      }
  ...
  // Worker Processes
    ...
    task1_buffer[0] = myRank;
    task1_buffer[1] = -1;
    while(true){

      // Send request for work
      MPI_Send(task1_buffer, N_IM+3, MPI_INT, 0, 100+myRank, MPI_COMM_WORLD);
